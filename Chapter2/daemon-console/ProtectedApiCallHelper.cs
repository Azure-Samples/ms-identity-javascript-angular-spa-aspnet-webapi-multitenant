...
namespace daemon_console
{
    ...
    public class ProtectedApiCallHelper
    {
        ...
        public async Task CallWebApiPostAndProcessResultASync(string webApiUrl, string accessToken, Action<JObject> processResult, StringContent data)
        {
            if (!string.IsNullOrEmpty(accessToken))
            {
                var defaultRequestHeaders = HttpClient.DefaultRequestHeaders;
                ...
                Console.WriteLine("***Content Location ****" + response.Headers.Location?.ToString());
                if (response.IsSuccessStatusCode)
                {
                    string json = await response.Content.ReadAsStringAsync();
                    JObject result = JsonConvert.DeserializeObject(json) as JObject;
        ...
        public async Task CallWebApiPutAndProcessResultASync(string webApiUrl, string accessToken, Action<JObject> processResult, StringContent data)
        {
            if (!string.IsNullOrEmpty(accessToken))
            {
                var defaultRequestHeaders = HttpClient.DefaultRequestHeaders;
